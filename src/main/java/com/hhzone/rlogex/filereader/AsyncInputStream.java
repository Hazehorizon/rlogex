...
public class AsyncInputStream implements ReadStream<Buffer> {
     ...
     * @param in
     */
    public AsyncInputStream(final Vertx vertx, final InputStream in, int threadsCount) {
      this.vertx = vertx;
      this.workerPool = vertx.createSharedWorkerExecutor("file-read-pool", threadsCount);
